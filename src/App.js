...
async function fetchCNPJ(cnpjStr) {
  try {
    const res = await fetch(`https://brasilapi.com.br/api/cnpj/v1/${cnpjStr}`);
    if (!res.ok) return null;
    const json = await res.json();
    return { rs: json.razao_social, nf: json.nome_fantasia || "", sit: json.descricao_situacao_cadastral, mun: json.municipio, uf: json.uf, t: json.identificador_matriz_filial === 1 ? 'Matriz' : 'Filial' };
  } catch (e) { return null; }
}
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        var yT = yt + 27; doc.setDrawColor(43, 76, 126); doc.setLineWidth(0.3); doc.line(c1, yT - 4, c6 + 1, yT - 4); doc.setFont("helvetica", "bold"); doc.setTextColor(43, 76, 126); doc.text("TOTAL ANUAL", c4, yT); doc.setTextColor(0); doc.text(fm(b.tR), c5, yT, { align: "right" }); doc.text(fm(b.tI), c6, yT, { align: "right" });
...
export default function App() {
  ...
  function mask(e) {
    ...
    if (dig.length === 14) {
      var r = vCNPJ(dig); setCVal(r);
      if (r.v) {
        var db = DB[dig];
        if (db) {
          setCData(db);
          setFp(function (p) { return Object.assign({}, p, { cnpj: dig, nome: db.rs }); });
        } else {
          ...
          fetchCNPJ(dig).then(apiData => {
            if (apiData) {
              setCErr("");
              setCData(apiData);
              setCVal(prev => prev ? Object.assign({}, prev, { t: apiData.t }) : null);
              setFp(p => Object.assign({}, p, { cnpj: dig, nome: apiData.rs }));
            } else {
              setCErr("CNPJ não encontrado na base de dados pública.");
              setCData(null);
              setFp(p => Object.assign({}, p, { cnpj: dig }));
            }
          });
        }
      } else setCErr(r.e);
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(145deg,#0d1b2a,#1b2a4a,#1a3a5c)", fontFamily: "'Segoe UI',system-ui", color: "#e0e6ed" }}>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "24px 20px" }}>
        <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
          {[{ n: 1, l: "Fonte Pagadora" }, { n: 2, l: "Importar Dados" }, { n: 3, l: "Gerar PDFs" }].map(function (it) {
            return (
        ...
        {step === 3 && <div style={S.card}>
          ...
          {bens.map(function (b, i) {
            return <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 14px", borderRadius: 8, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", marginBottom: 7 }}>
