...
async function fetchCNPJ(cnpjStr) {
  try {
    ...
    return {
      rs: json.razao_social,
      nf: json.nome_fantasia || "",
      sit: json.descricao_situacao_cadastral,
      mun: json.municipio,
      uf: json.uf,
      t: json.identificador_matriz_filial === 1 ? 'Matriz' : 'Filial'
    };
  } catch (e) { return null; }
}
  ...
  function mask(e) {
    ...
    if (dig.length === 14) {
      ...
      if (r.v) {
        ...
        if (db) {
          setCData(db);
          setFp(p => Object.assign({}, p, { cnpj: dig, nome: db.rs }));
        } else {
          setCErr("Buscando dados do CNPJ...");
          fetchCNPJ(dig).then(apiData => {
            if (apiData) {
              setCErr("");
              setCData({ rs: apiData.rs, sit: apiData.sit });
